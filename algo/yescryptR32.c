...
int yescrypt_client_key;
int yescrypt_client_key_len;
...
static int yescrypt_WaviBanana(const uint8_t *passwd, size_t passwdlen,
...
extern void yescryptR32_hash(const char *input, char *output)
{
    yescrypt_WaviBanana((const uint8_t *) input, 80,
...
int scanhash_yescryptR32(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
                      uint32_t max_nonce, unsigned long *hashes_done)
{
//        uint32_t data[20] __attribute__((aligned(64)));
//        uint32_t hash[8] __attribute__((aligned(64)));
	uint32_t _ALIGN(64) hash[8];
        uint32_t _ALIGN(64) data[32];
        ...
        do {
                be32enc(&data[19], ++n);
                yescryptR32_hash((char *)data, (char *)hash);
