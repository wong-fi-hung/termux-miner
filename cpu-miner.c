...
Options:\n\
  -a, --algo=ALGO       specify the algorithm to use\n\
			  ...
			  bmw512        BMW 512 (KONJ & XDN)\n\
                          ...
                          cryptonight   CN v8 (Monero)\n\
			  curve         CURVE (Curve coin)\n\
                          ...
                          keccakc       Keccak (GlobalToken CreativeCoin)\n\
                          luffa         Luffa\n\
                          lyra2re       Lyra2RE\n\
                          lyra2rev2     Lyra2REv2\n\
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		case ALGO_YESPOWER:
			...
			break;
		case ALGO_YESPOWERR16:
			rc = scanhash_yespowerR16(thr_id, work.data, work.target, max_nonce, &hashes_done);
			break;
		case ALGO_YESPOWERLITB:
			rc = scanhash_yespowerLITB(thr_id, work.data, work.target, max_nonce, &hashes_done);
			break;
		case ALGO_YESPOWERLNC:
			rc = scanhash_yespowerLNC(thr_id, work.data, work.target, max_nonce, &hashes_done);
			break;
		case ALGO_YESPOWERIC:
			rc = scanhash_yespowerIC(thr_id, work.data, work.target, max_nonce, &hashes_done);
			break;
		case ALGO_YESPOWERIOTS:
			rc = scanhash_yespowerIOTS(thr_id, work.data, work.target, max_nonce, &hashes_done);
			break;
		case ALGO_YESPOWERITC:
			rc = scanhash_yespowerITC(thr_id, work.data, work.target, max_nonce, &hashes_done);
			break;
		case ALGO_YESPOWERSUGAR:
...
static void show_version_and_exit(void)
{
	...
	printf(" config features:"
		...
		" AVX2"
#endif
#if defined(__ARM_ARCH_8A__) || defined(__ARM_ARCH_8__)
		"ARMv8A"
#endif
